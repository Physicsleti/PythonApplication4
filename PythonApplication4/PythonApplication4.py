...
Holex = curve(pos=[(0,0,0), (L*rel,0,0)], color=color.green)
Holey = curve(pos=[(0,0,0), (0,L*rel,0)], color=color.green)
Holex1 = curve(pos=[(0,L*rel,0), (L*rel,L*rel,0)], color=color.green)
Holey1 = curve(pos=[(L*rel,0,0), (L*rel,L*rel,0)], color=color.green)
...
while True:
    ...
    pval=p
    p1 = p*outside
    p = p-p1+abs(p1)* loss # force p component inward
    ...
    for i in range(Natoms):
        out=0
        if Atoms[i].pos.z<=Ratom and Atoms[i].pos.x<rel*L and Atoms[i].pos.y<rel*L:
                  out=1
                  Atoms[i].pos.z=-2*Ratom
                  Atoms[i].pos.x=rel/2*L
                  Atoms[i].pos.y=rel/2*L
                  Atoms[i].radius=0
                  Atoms[i].parameter1=0

        if out==0:          
                  Atoms[i].pos = pos[i]
        #val=255#    int(mag(p[i])/(3*pavg)*255);
        #Atoms[i].color = getRGBfromI(val)
